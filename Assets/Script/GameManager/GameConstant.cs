
public class GameConstant 
{
    public const string GAME_SAVE = "GameSave";
    public const string NEW_GAME = "NEW_GAME";
    public const string LOAD_GAME = "LOAD_GAME";
    public const string GAME_SAVE_PLAYERSTAT = "GAME_SAVE_PLAYERSTAT";
    public const string GAME_SAVE_INVENTORY = "GAME_SAVE_INVENTORY";
    public const string GAME_SAVE_SCENE = "GAME_SAVE_SCENE";
    public const string INVENTORY_DATA = "InventoryData";
    public const string FARMTILES_DATA = "FarmTilesData";
    public const string CURRENTWEAPON_DATA = "CURRENTWEAPON_DATA";
    public const string FOREST_DATA_JSON = "forest_data.json";
    public const string NPC_FEATURES_SHOP = "NPC_FEATURES_SHOP";
    public const string UI_LOADSCENE = "UI_LOADSCENE";
    public const string VOLUME_MUSIC = "VOLUME_MUSIC";
    public const string VOLUME_SOUND = "VOLUME_SOUND";
}
